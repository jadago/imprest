                ...
                <div class="col-md-12">
                    ...
                    <div class="panel panel-info">
                      ...
                      <div class="panel-heading">
                        <h6 class="panel-title">Un-retired Report</h6>
                      </div>
...
<tr>
  ...
  <td><div class="col-sm-4">
    <select  class="select" name="item" style="width:300px;">
      <option value="0">All Staff</option>
      <?php 
									require_once('includes/db_conn.php');
									$item = "SELECT * FROM users WHERE user_id IN (SELECT addedby FROM imprest WHERE status=5 AND stage < 4) ORDER BY firstname ASC";
...
<tr>
  ...
  <td><span class="col-sm-4">
    <select  class="select" name="department" style="width:300px;">
      <option value="0">All Department</option>
      <?php 
									require_once('includes/db_conn.php');
									$item2 = "SELECT * FROM departments ORDER BY department_id ASC";
									$queryitem2 = mysqli_query($con,$item2);
									while ( $arrayitem2 = mysqli_fetch_array($queryitem2))
									{
									?>
      <option value="<?php echo $arrayitem2['department_id']; ?>"><?php echo $arrayitem2['department_name'];?></option>
      <?php
									}
									?>
    </select>
		...
		?>
                      ...
                      <table class="table table-bordered table-striped datatable-selectable" width="80%">
                               ...
                               require_once('includes/db_conn.php');
								...
								if($_POST['department'] > 0) $depart= "AND addedby IN(SELECT user_id FROM users WHERE department='".$_POST['department']."')"; else $depart="";
